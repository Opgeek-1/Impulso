...
const STATUS_MAP: Record<string, { labelKey: string; color: string; bg: string }> = {
  ...
  IMAGE_GENERATED: { labelKey: "image_ready", color: "var(--s-image)", bg: "var(--s-image-bg)" },
...
export function TweetPreviewModal({ tweet, project, onClose, onPublished }: TweetPreviewModalProps) {
  ...
  async function publishNow() {
    ...
    if (res.ok) {
      toast.success(t("posted_to_x"));
      onPublished?.(data);
      onClose();
    } else {
      toast.error(data.error || t("publish_failed"));
    }
    setPublishing(false);
  }
      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-[13px] font-semibold" style={{ color: "var(--imp-text)" }}>{t("post_preview")}</span>
            <span
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[13.5px]" style={{ color: "var(--imp-faint)" }}>· {tweet.status === "POSTED" ? ts("posted") : tweet.status === "SCHEDULED" ? ts("scheduled") : ts("draft")}</span>
              </div>
            </div>
          </div>
